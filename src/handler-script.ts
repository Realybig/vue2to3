...
function Vue2ToCompositionApi(
  ...
  try {
    ...
    // 修改为
    globalObj.Vue2ToCompositionApiVmBody = {};
    ...
    if (modelScriptContent) {
      eval(`globalObj.Vue2ToCompositionApiVmBody = ${modelScriptContent}`);
    } else {
      throw new Error(`Vue2ToCompositionApi 内容不是有效内容`);
    }

    // 初步处理后的结果
    const vmBody: any = globalObj.Vue2ToCompositionApiVmBody;
